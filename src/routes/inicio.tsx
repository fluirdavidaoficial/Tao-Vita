...
function Inicio() {
  return (
    <>
      ...
      <h1 className="font-display text-[clamp(1.85rem,7vw,2.5rem)] leading-tight">
        O que você quer estudar
      </h1>
      <p className="mt-1 text-base text-muted">Um caminho por vez. Tudo volta aqui.</p>

      <nav className="stagger-list mt-5 space-y-2.5">
        <MenuRow
          to="/atlas"
          icon={BookOpen}
...
function MenuRow({
  ...
  return (
    <Link
      ...
      className={cn(
        "flex min-h-24 items-center gap-4 rounded-2xl bg-surface px-4 py-4 shadow-[var(--shadow-border)]",
        "transition-[transform,box-shadow] duration-150 ease-out",
        "hover:shadow-[var(--shadow-border-hover)] active:scale-[0.99]",
      )}
    >
      <span className="grid size-14 shrink-0 place-items-center rounded-2xl bg-bg text-primary">
      ...
      <ChevronRight className="size-5 shrink-0 text-subtle" strokeWidth={1.75} />
    </Link>
  );
}
